...
test('event API returns all events when no date filter is provided', () => {
  const params = getEventRangeParams(new URL('https://example.com/api/events').searchParams);

  assert.deepEqual(params, { mode: 'all' });
});
